...
import Tile from '../../../daily_co/components/Tile';
import { DEFAULT_ASPECT_RATIO } from '../../../daily_co/constants';
...
export const VideoGrid = React.memo(() => {
    ...
    const tiles = useDeepCompareMemo(() =>
      participants.map((p) => (
        <Tile
          participant={p}
          showRole={context?.game_config?.players.filter((player) => player.player_id === session?.user?.info?._id)[0]?.role}
          photo={context?.game_config?.players.filter((player) => player.player_id === p?.user_id)[0]?.player_photo_url}
          name={context?.game_config?.players.filter((player) => player.player_id === p?.user_id)[0]?.player_name}
          role={context?.game_config?.players.filter((player) => player.player_id === p?.user_id)[0]?.role}
          key={p.id}
          mirrored

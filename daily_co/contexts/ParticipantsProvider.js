...
export const ParticipantsProvider = ({ children }) => {
  ...
  const [userId, setUserId] = useState(callObject?.participants()?.local?.user_id)
  ...
  const userid = callObject?.participants()?.local?.user_id ?? '';
  ...
  const handleNewParticipantsState2 = useCallback((event = null) => {
    switch (event?.action) {
      ...
      case 'participant-left':
        dispatch({
          ...
          participant: event.participant,
          session: event.participant.session_id,
        });
        break;
      default:
        break;
  ...
  const handleNewParticipantsState = useCallback((event = null) => {
    switch (event?.action) {
      case 'participant-joined':
        // If the new participant's user id is the same as local participant's user id,
        // leave the local participant's session. Othersise, join the new participant.
        if (!localParticipant.isOwner && event.participant.user_id === callObject?.participants()?.local?.user_id) {
          // If the local participant is older than the new participant,
          // which has same user id, leave it from the video chat.
          if (event.participant.joined_at > callObject?.participants()?.local?.joined_at) {
            callObject.leave();
          }
        }
        else{
